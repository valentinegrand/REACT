...
### POur pouvoir gérer les titres compliqués on ne met pas de {title} et on laisse postamn gérer les caractères : by_title?title=...
...
@app.get("/stars/by_birthdate/{year}", response_model=List[schemas.Star])
def read_star_by_birthyear(year: Optional[int], db: Session = Depends(get_db)):
    # read items from database
    star = crud.get_stars_by_birthdate(db=db, year=year)
    # return them as json
    return star
